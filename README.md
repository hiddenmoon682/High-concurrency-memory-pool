...
基数树加 `-DTC_USE_RADIX_PAGEMAP=1`（默认）、对照加 `-DTC_USE_RADIX_PAGEMAP=0`。
...
alloc 一侧同样受益（对照实现的 `MapObjectToSpan` 在自由链表归还路径上要加内部锁）。
单线程下基数树的 free 也比对照快约 5 倍：对照实现每次查找都要走一次互斥锁 + 哈希表探测。
